...
var player_entity_data = preload("res://resources/custom_resources/entities/player.tres")
...
func _enter_state(new_state: LevelManagerState):
	match new_state:
		LevelManagerState.ENTERING_LEVEL:
			...
			print("requesting transition open")
...
func set_level(new_level_data: LevelData):
	...
	# Save level state data in game data
	if current_level_state_data:
		emit_signal("level_exited", current_level_state_data)
	...
	if new_level_saved_level_state:
		current_level_state_data = new_level_saved_level_state
	else:
...
func spawn_level(level_data: LevelData, level_state_data: LevelStateData):
	...
	var new_level_node = level_data.scene.instantiate()
	
	#var player_starting_data_idx = level_data.player_start_data_from_level.find_custom(func(player_starting_data):
		#return player_starting_data.coming_from_level == previous_level_data
	#)
	#var player_starting_data = level_data.player_start_data_from_level[player_starting_data_idx]
	
	level_container.add_child(new_level_node)
	current_level_node = new_level_node
	
	var player_node = player_entity_data.scene.instantiate()
